...
const AddTestPanel = () => {
  ...
  const onSubmit = data => {
    const matrixColumns = data.matrixTestReportTemplate.columns.map(column => {
        ...
        const c = column.inputComment;
        const c = column.inputComment;
        return {
            ...column,
            [i]: c, // Use computed property name to set the dynamic key-value pair
      ...
      <div>
        ...
        {columnFields.map((column, columnIndex) => (
          <div key={column.id} className="grid grid-cols-2 gap-4">
            ...
            <label htmlFor="matrixTestReportTemplate.columns.${columnIndex}.inputC" className="block text-sm font-medium text-gray-700">columnName</label>
            <input {...register(`matrixTestReportTemplate.columns.${columnIndex}.inputComment`)} className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" />
