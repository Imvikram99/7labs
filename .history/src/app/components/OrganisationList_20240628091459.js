...
const AddTestPanel = () => {
  ...
  const { fields: columnFields, append: appendColumn, remove: removeColumn } = useFieldArray({ control, name: `tests.${fields.length - 1}.matrixTestReportTemplate.columns` });
  const { fields: columnStyleFields, append: appendColumnStyle, remove: removeColumnStyle } = useFieldArray({ control, name: `tests.${fields.length - 1}.matrixTestReportTemplate.columnStyles` });
      ...
      {testResultType === 'BLOOD_MULTIPLE_PARAMETER' ? (
        <>
          {fields.map((item, index) => {
            const referenceValueType = watch(`tests.${index}.referenceValueType`);
            return (
              <div key={item.id} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {referenceValueType === 'RANGE' && (
                    <div className="col-span-2 space-y-4">
                      <button
                        type="button"
                        onClick={() => append({ minAge: '', maxAge: '', gender: '', minReferenceValue: '', maxReferenceValue: '', testResultUnit: '' })}
                        className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                      >
                        Add Reference Value Set
                      </button>
                      {item.referenceValues?.map((refValue, refIndex) => (
                        <div key={refIndex} className="space-y-4">
                          <label htmlFor={`tests.${index}.referenceValues.${refIndex}`} className="block text-sm font-medium text-gray-700">Reference Values</label>
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <label htmlFor={`tests.${index}.referenceValues.${refIndex}.minAge`} className="block text-sm font-medium text-gray-700">Min Age</label>
                              <input type="number" {...register(`tests.${index}.referenceValues.${refIndex}.minAge`)} className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" />
                            </div>
                            <div>
                              <label htmlFor={`tests.${index}.referenceValues.${refIndex}.maxAge`} className="block text-sm font-medium text-gray-700">Max Age</label>
                              <input type="number" {...register(`tests.${index}.referenceValues.${refIndex}.maxAge`)} className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" />
                            </div>
                            <div>
                              <label htmlFor={`tests.${index}.referenceValues.${refIndex}.gender`} className="block text-sm font-medium text-gray-700">Gender</label>
                            ...
                            <div>
                              <label htmlFor={`tests.${index}.referenceValues.${refIndex}.minReferenceValue`} className="block text-sm font-medium text-gray-700">Min Reference Value</label>
                              <input type="number" {...register(`tests.${index}.referenceValues.${refIndex}.minReferenceValue`)} className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" />
                            </div>
                            <div>
                              <label htmlFor={`tests.${index}.referenceValues.${refIndex}.maxReferenceValue`} className="block text-sm font-medium text-gray-700">Max Reference Value</label>
                              <input type="number" {...register(`tests.${index}.referenceValues.${refIndex}.maxReferenceValue`)} className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" />
                            </div>
                            <div>
                              <label htmlFor={`tests.${index}.referenceValues.${refIndex}.testResultUnit`} className="block text-sm font-medium text-gray-700">Test Result Unit</label>
                              <input {...register(`tests.${index}.referenceValues.${refIndex}.testResultUnit`)} className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" />
                            </div>
                          </div>
                          <button
                            type="button"
                            onClick={() => remove(refIndex)}
