...
export default function RegisterCustomer() {
  ...
  const determineSearchType = (query) => {
    if (query.includes('@')) {
      return "email";
    } else if (query.match(/^\+?\d{1,3}?[\s-]?\d{4,10}$/)) {
      return "phone";
    } else {
      return "patientId"; // Default to patient ID if the query isn't clearly an email or phone number
    }
  };
  ...
  const handleSubmit = (event) => {
    ...
    console.log(fieldsToSend); // Placeholder for your submission logic
