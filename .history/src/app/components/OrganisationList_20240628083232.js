...
const AddTestPanel = () => {
  ...
  const { fields: referenceValueFields, append: appendReferenceValue, remove: removeReferenceValue } = useFieldArray({ control, name: `tests.${index}.referenceValues` });        <div key={item.id} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
  ...
  <button type="button" onClick={() => removeReferenceValue(refIndex)}>Remove This Set</button>
</div>
))}
