...
const AddTestPanel = () => {
  ...
  const { fields: columnFields, append: appendColumn, remove: removeColumn } = useFieldArray({ control, name: `tests.0.matrixTestReportTemplate.columns` }); // Update name for a specific test index
  const { fields: columnStyleFields, append: appendColumnStyle, remove: removeColumnStyle } = useFieldArray({ control, name: `tests.0.matrixTestReportTemplate.columnStyles` }); // Update name for a specific test index

  const [testCategories, setTestCategories] = useState([]);
  const [testUnits, setTestUnits] = useState([]);
  const testReferenceValueTypes = fields.map((field, index) => watch(`tests.${index}.referenceValueType`));
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                </div>
                {
                  testReferenceValueTypes[index] === 'RANGE' && (
                    <div>
                      <button type="button" onClick={() => append({ referenceValues: {} })}>Add Reference Value Set</button>
                      ...
                      <button type="button" onClick={() => remove(index)} className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">Remove This Set</button>
                    </div>
                  )
                }
              </div>
