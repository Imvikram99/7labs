...
const AddTestPanel = () => {
  ...
  const onSubmit = data => {
    const matrixColumns = data.matrixTestReportTemplate.columns.map(column => {
        return {
          ...column,
          inputKey: `Key: ${column.inputKey}`, // Modify the key
          inputComment: `Comment: ${column.inputComment}`, // Modify the comment
          columnName: `Name: ${column.columnName}` // Modify the name, assuming columnName is the correct property
        };
      });
    const formData = {
      ...data,
